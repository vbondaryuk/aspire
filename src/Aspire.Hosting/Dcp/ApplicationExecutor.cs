...
#pragma warning restore CS0612
    ...
    private void AddServicesProducedInfo(IResource modelResource, IAnnotationHolder dcpResource, AppResource appResource)
    {
        ...
        foreach (var sp in servicesProduced)
        {
            ...
            {
                if (appResource.DcpResource is ExecutableReplicaSet)
                {
                    throw new InvalidOperationException($"Resource '{modelResourceName}' can have multiple replicas and uses a proxy-less endpoint '{ea.Name}'. These features do not work together.");
                }
